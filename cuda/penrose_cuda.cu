    ...
    int n_tris)                              // input triangle count
{
    ...
    if (types[idx] == 0) {
        ...
        // Simplified: write to positions 0, 1, 2 (single block — DEBUG ONLY)
        // FM: For real multi-block, use prefix-sum atomic counters.
        // I've kept the SIMPLE version here so you can hack the subdivision
        // math without worrying about memory layout.

        // For now, skip thin to avoid hard-to-debug offsets
        // FM: Replace this with proper atomicAdd-based prefix sum
        return;
    }
}
